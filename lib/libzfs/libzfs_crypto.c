...
static int get_key_material(libzfs_handle_t *hdl, key_format_t format, key_locator_t locator, uint8_t **key_material_out, size_t *key_material_len){
	int ret;
	int rbytes;
	...
	*key_material_len = 0;
	...
	case KEY_LOCATOR_PROMPT:
		if (format == KEY_FORMAT_RAW) {
			...
			errno = 0;
			rbytes = read(STDIN_FILENO, key_material, WRAPPING_KEY_LEN);
...
static int derive_key(libzfs_handle_t *hdl, key_format_t format, uint8_t *key_material, size_t key_material_len, uint64_t salt, uint8_t **key_out){
	int ret;
	uint8_t *key;
...
static int populate_create_encryption_params_nvlist(libzfs_handle_t *hdl, char *keysource, nvlist_t *props) {
	int ret;
	uint64_t salt = 0;
	...
	ret = get_key_material(hdl, keyformat, keylocator, &key_material, &key_material_len);
	if (ret)
		goto error;
	...
	ret = derive_key(hdl, keyformat, key_material, key_material_len, salt, &key_data);
	if (ret)
		goto error;
	
	/* add the derived key to the properties list */
	ret = nvlist_add_uint8_array(props, "wkeydata", key_data, WRAPPING_KEY_LEN);
	if (ret)
		goto error;
...
int zfs_crypto_create(libzfs_handle_t *hdl, nvlist_t *props, char *parent_name) {	
	...
	if (pzhp == NULL) {
		zfs_error_aux(hdl, dgettext(TEXT_DOMAIN, "Failed to lookup parent."));
		return (ENOENT);
	}
...
int zfs_crypto_clone(libzfs_handle_t *hdl, zfs_handle_t *origin_zhp, nvlist_t *props, char *parent_name, boolean_t add_key) {
	...
	if (ret == 0) {
		ret = EINVAL;
		zfs_error_aux(hdl, dgettext(TEXT_DOMAIN, "Encryption may not be specified during cloning."));
		goto out;
	}
	...
	if (pcrypt != ZIO_CRYPT_OFF && ocrypt == ZIO_CRYPT_OFF) {
		ret = EINVAL;
		zfs_error_aux(hdl, dgettext(TEXT_DOMAIN, "Cannot create unencrypted clone as child of encrypted parent."));
		goto out;
	}
	...
	/* by this point this dataset will be encrypted. if the parent doesn't have a keysource to inherit we need one provided */
...
int zfs_crypto_load_key(zfs_handle_t *zhp) {
	...
	(void) snprintf(errbuf, sizeof (errbuf), dgettext(TEXT_DOMAIN, "Key load error"));
	...
	ret = zfs_prop_get(zhp, ZFS_PROP_KEYSOURCE, keysource, sizeof (keysource), &keysource_srctype, keysource_src, sizeof (keysource_src), B_TRUE);
	if (ret) {
		...
		zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Keys must be loaded for encryption root '%s'."), keysource_src);
		ret = EINVAL;
		goto error;
	}
	...
	/* get key material from keysource */
	ret = get_key_material(zhp->zfs_hdl, format, locator, &key_material, &key_material_len);
	if (ret)
		goto error;
	...
	/* derive a key from the key material */
	ret = derive_key(zhp->zfs_hdl, format, key_material, key_material_len, salt, &key_data);
	if (ret)
		goto error;
	...
	ret = nvlist_add_uint8_array(nvl, "wkeydata", key_data, WRAPPING_KEY_LEN);
	if (ret)
		goto error;
	...
	switch(ret){
		case EINVAL:
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Incorrect key provided."));
			break;
		case EEXIST:
...
int zfs_crypto_unload_key(zfs_handle_t *zhp) {
	...
	if (ret) {
		...
		zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Keys must be unloaded for encryption root '%s'."), keysource_src);
		ret = EINVAL;
		goto error;
	}
	...
	if(ret){
		switch(ret){
		case ENOENT:
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Keychain is not currently loaded."));
			break;
		case EBUSY:
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Dataset is busy."));
			break;
...
int zfs_crypto_add_key(zfs_handle_t *zhp) {
	int ret;
	char errbuf[1024];
	uint64_t crypt;
	
	(void) snprintf(errbuf, sizeof (errbuf), dgettext(TEXT_DOMAIN, "Add key error"));
...
int zfs_crypto_rewrap(zfs_handle_t *zhp, nvlist_t *props) {
	int ret;
	char errbuf[1024];
	uint64_t crypt;
	...
	(void) snprintf(errbuf, sizeof (errbuf), dgettext(TEXT_DOMAIN, "Rewrap keychain error"));
	...
	ret = nvlist_lookup_string(props, zfs_prop_to_name(ZFS_PROP_KEYSOURCE), &keysource);
	if (ret == ENOENT) {
		ret = zfs_prop_get(zhp, ZFS_PROP_KEYSOURCE, prop_keysource, sizeof (prop_keysource), NULL, NULL, 0, B_TRUE);
		if (ret) {
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Failed to obtain keysource property."));
			ret = EIO;
			goto error;
		...
		zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Failed to find keysource."));
		ret = EIO;
		goto error;
	}
	
	/* populate the nvlist with the encryption params */
	ret = populate_create_encryption_params_nvlist(zhp->zfs_hdl, keysource, props);
	if (ret)
		goto error;
	...
	if(ret){
		switch(ret){
		case EINVAL:
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Invalid properties for key change."));
			break;
		case ENOENT:
			zfs_error_aux(zhp->zfs_hdl, dgettext(TEXT_DOMAIN, "Keychain is not currently loaded."));
			break;
